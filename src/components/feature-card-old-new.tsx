...
export function FeatureCardOldNew({
  ...
  return (
    <div
      className={cn(
        "relative h-[80vh] overflow-hidden rounded-[2rem] border border-border bg-card shadow-sm",
        className
      )}
    >
      <div className="h-full px-8 py-4 md:px-10 md:py-6">
        <div className="grid h-full grid-cols-1 md:grid-cols-2 gap-12 md:gap-14 lg:gap-16 items-stretch text-left">
          <div className={cn("flex h-full min-h-0 flex-col gap-6 md:gap-7", textOrderClassName)}>
            {eyebrow ? (
              <p className="m-0 shrink-0 text-base leading-relaxed text-white">
                {eyebrow}
              </p>
            ) : null}
            <div className="grid min-h-0 flex-1 grid-rows-2 gap-6 md:gap-7">
            <div
              onMouseEnter={() => {
                setHoveredChoice("old");
                setIsHovered(true);
              }}
              onMouseLeave={() => {
              ...
              className={cn(
                "relative flex min-h-0 h-full cursor-crosshair flex-col rounded-2xl bg-primary/5 p-5 transition-[opacity,box-shadow] duration-200 hover:shadow-[0_16px_40px_-12px_hsl(var(--foreground)/0.35)]",
                previewState === "new"
                  ? "opacity-45"
            ...
            >
              ...
              <h4 className="whitespace-pre-line font-space-grotesk text-xl sm:text-2xl font-bold tracking-tight">
                {oldWay.title}
              </h4>
              {oldWay.subtitle ? (
            ...
            <div
              ...
              onBlur={() => setHoveredChoice(null)}
              className={cn(
                "relative flex min-h-0 h-full cursor-crosshair flex-col rounded-2xl bg-background/60 p-5 transition-[opacity,box-shadow] duration-200 hover:shadow-[0_16px_40px_-12px_hsl(var(--foreground)/0.35)]",
                previewState === "old"
                  ? "opacity-45"
                  : "opacity-100 shadow-[0_16px_40px_-12px_hsl(var(--foreground)/0.35)]"
              )}
          ...
          >
            <div
              className={cn(
                "relative h-full min-h-[220px] w-full transition-colors duration-200",
                previewState === "old"
                  ? "bg-primary/5"
                  : "bg-background/60"
              )}
