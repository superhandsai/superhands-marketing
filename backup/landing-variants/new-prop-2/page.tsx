...
export const metadata: Metadata = {
  title: "Superhands — Design review for AI-powered teams that move fast",
  description:
    "Catch design issues earlier, flag uncertainty in PRs, and turn repeated feedback into reusable guidance that strengthens your design system.",
};
...
const fluid = {
  ...
  leadTitle: "clamp(24px, 18px + 1.2vw, 34px)",
...
function CtaPair({ stacked = false }: { stacked?: boolean }) {
  // When stacked, both buttons take the width of the widest one (align-stretch
  // on an inline-flex column), so the primary CTA matches the secondary.
  const layout = stacked
    ? "inline-flex flex-col items-stretch gap-3"
...
const FEATURES: Feature[] = [
  {
    title: "Catch issues before they become expensive",
    body: "Superhands evaluates UI work as it takes shape and surfaces likely design issues before they turn into costly rework.",
  },
  {
    title: "Flag uncertainty in PRs, not in people\u2019s heads",
    body: "When the system is unsure, it doesn\u2019t stop the flow. It creates clear annotations in PRs so reviewers can quickly see where human judgment is needed.",
  },
  {
    title: "Turn review into reusable design system guidance",
    body: "Every review comment is a chance to improve the system. Superhands captures repeated feedback and turns it into guidance the next builder can use.",
  },
  {
    title: "Help every builder benefit from the design system",
    body: "Engineers, PMs, designers, and agents all get access to evolving, team-specific guidance \u2014 not just static docs.",
  },
  {
    title: "Strengthen your design system through real product work",
    body: "Instead of forcing teams to define everything upfront, Superhands creates a feedback loop where every product decision feeds back into your design system.",
  },
];
...
function LeadFeatureCard({ feature, index }: { feature: Feature; index: number }) {
  return (
    <div className="flex flex-col gap-6 rounded-[25px] bg-[#0D0D0D] p-8 outline outline-1 outline-white/15 md:col-span-2 md:p-10">
      <Eyebrow>{`0${index + 1}`}</Eyebrow>
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:gap-16">
        <h3
          className="font-medium leading-[120%] md:flex-1"
          style={{
            ...gradientText,
            fontFamily: "var(--font-roobert)",
            fontSize: fluid.leadTitle,
          }}
        >
          {feature.title}
        </h3>
        <p
          className="font-normal leading-[150%] text-[#D9D9D9] md:flex-1"
...
export default function NewProp2Page() {
  return (
    <div
    ...
    >
      ...
      >
        <div className="mx-auto flex w-full max-w-[1280px] flex-col gap-[60px] px-6 pt-[60px] md:px-[60px]">
          ...
          <div className="flex flex-col items-start justify-between gap-10 lg:flex-row lg:items-center lg:gap-20">
            ...
            >
              Design review for AI-powered teams that move fast
            </h1>
            <div className="flex w-full max-w-[400px] flex-col items-start justify-center gap-4">
              <p
                className="font-medium leading-[135%] text-[#D9D9D9]"
                style={{ fontSize: fluid.subhead }}
              >
                Catch design issues earlier, flag uncertainty in PRs, and turn
                repeated feedback into reusable guidance that strengthens your
                design system.
              </p>
              <CtaPair stacked />
            </div>
      ...
      <section className="mx-auto flex w-full max-w-[1280px] flex-col items-center gap-10 px-6">
        ...
        <div className="flex max-w-[640px] flex-col items-center gap-6">
          <p
            ...
            style={{ fontFamily: "var(--font-roobert)", fontSize: fluid.body }}
          >
            Most design systems document components, not the real-world
            decisions that happen during implementation. So feedback comes late,
            designers become bottlenecks, and the same guidance gets repeated
          ...
          <p
            ...
            style={{ fontFamily: "var(--font-roobert)", fontSize: fluid.body }}
          >
            Superhands turns design review into a feedback loop that helps teams
            move fast without losing quality or consistency.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto flex w-full max-w-[1280px] flex-col gap-10 px-6 md:px-[60px]">
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          {FEATURES.map((feature, i) =>
            i === 0 ? (
              <LeadFeatureCard key={i} feature={feature} index={i} />
            ) : (
      ...
      {/* Closing CTA */}
      <section className="mx-auto flex w-full max-w-[1280px] flex-col gap-8 px-6 md:px-[120px]">
        <div className="flex max-w-[760px] flex-col gap-6">
          <h2
            className="font-medium leading-[115%]"
            style={{
              ...gradientText,
              fontFamily: "var(--font-roobert)",
              fontSize: fluid.section,
